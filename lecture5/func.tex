...
\begin{itemize}
	...
	\item {The input parameter(s) along with the types should be specified}
	\item {The output value type should be specified}
\end{itemize}
\end{frame}
...
\begin{itemize}
	...
	\item {If there is nothing, the returning type is \textcolor{blue}{int} by default (\textcolor{red}{this is not recommended!!})}
\end{itemize}
\end{frame}
...
\begin{itemize}
	...
	\item {There should be input parameter(s) along with the types}
	\item {There should be output value type}
	\item {If there is nothing, the return type is \textcolor{blue}{int} by default}
\end{itemize}
\end{frame}
...
\end{itemize}
\begin{lstlisting}
float area(float r, const float pi)
{
   float a = 0;
   a = r*r*pi;
}
...
\end{column}
\begin{column}{0.48\linewidth}
\begin{lstlisting}[firstnumber=21, linewidth=0.95\linewidth]
    s += t;
    a  = a/10;
  }
  if(s == n)
...
\section{Parameter Transfer in C Functions}
