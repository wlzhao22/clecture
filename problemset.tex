...
\begin{enumerate}
    ...
    \item Define a function pointer pointing to functions such as sin(x), $x^2$, $e^x$ that are defined by yourself as well. It is allowed the user to choose based on which to calculate its integral between range $[a, b]$, the parameters a and b are also specified by the user. The code should print out the integral value for the specified function and integral range.
\end{enumerate}
...
\end{lstlisting}	
	...
	\item {Try the above problem by composing a ``CMakeLists.txt'' file manually to compile the project.}
\end{enumerate}
